...
class JobPostForm(forms.Form):
    ...
    job_type = forms.ChoiceField(choices=JOB_TYPE, required=True, label="Job Type",
                                 widget=forms.Select(attrs={'class': 'form-control'}))
    ...
    closing_Date = forms.DateField(
        widget=forms.DateInput(attrs={'type': 'date', 'class': 'form-control'}, format='%Y-%m-%d'), required=True,
        label="Job Expiry Date", input_formats=('%Y-%m-%d',))
